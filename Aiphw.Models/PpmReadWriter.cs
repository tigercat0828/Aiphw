...
public static class PpmReadWriter {
    ...
    public static Bitmap ReadPPM(string filename) {
        try {
            ...
            string[] dimensions = reader.ReadLine()?.Trim().Split([' ', '\t'], StringSplitOptions.RemoveEmptyEntries);
            ...
            if (!int.TryParse(maxColorValue, out int maxValue) || maxValue <= 0 || maxValue > 255) {
            ...
            while (!reader.EndOfStream) {
                ...
                string[] pixelTokens = line?.Split([' ', '\t'], StringSplitOptions.RemoveEmptyEntries);
